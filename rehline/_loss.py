...
class ReHLoss:
    ...
    rehu_intercept : {array-like} of shape (n_rehu, n_samples)
        ReHU intercept matrix, where `n_rehu` is the number of ReHU terms and
    ...
    def __init__(
        ...
        if rehu_coef is None:
            rehu_coef = np.empty(shape=(0, 0))
        if rehu_intercept is None:
            rehu_intercept = np.empty(shape=(0, 0))
    ...
    def __call__(self, x):
        ...
        if (self.L > 0) and (self.H > 0):
            if self.relu_coef.shape[1] != self.rehu_coef.shape[1]:
                raise ValueError("n_samples for `relu_coef` and `rehu_coef` should be the same shape!")

        _check_relu(self.relu_coef, self.relu_intercept)
        _check_rehu(self.rehu_coef, self.rehu_intercept, self.rehu_cut)

        self.L, self.H, self.n = (
            self.relu_coef.shape[0],
            self.rehu_coef.shape[0],
